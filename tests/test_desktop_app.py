...
def test_desktop_app_opens_native_window_by_default(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ageos.app.desktop._launch_tauri_app", fake_launch)
...
def test_tauri_command_prefers_configured_binary(monkeypatch) -> None:
    monkeypatch.setenv("AGEOS_TAURI_BIN", "/opt/ageos/bin/ageos-control-center")

    assert _tauri_command() == ["/opt/ageos/bin/ageos-control-center"]


def test_tauri_command_prompts_to_install_when_missing(monkeypatch, tmp_path) -> None:
    installed = tmp_path / "ageos-control-center"
    monkeypatch.delenv("AGEOS_TAURI_BIN", raising=False)
    monkeypatch.setattr("ageos.app.desktop.shutil.which", lambda name: None)
    monkeypatch.setattr("ageos.app.desktop.Path.is_file", lambda self: False)
    monkeypatch.setattr("ageos.app.desktop._prompt_and_install_tauri_app", lambda: installed)

    assert _tauri_command() == [str(installed)]


def test_prompt_and_install_respects_explicit_no(monkeypatch) -> None:
    monkeypatch.setenv("AGEOS_INSTALL_APP", "0")
    install = Mock()
    monkeypatch.setattr("ageos.app.desktop._install_tauri_app", install)
...
def test_prompt_and_install_respects_explicit_yes(monkeypatch, tmp_path) -> None:
    installed = tmp_path / "ageos-control-center"
    monkeypatch.setenv("AGEOS_INSTALL_APP", "yes")
    monkeypatch.setattr("ageos.app.desktop._install_tauri_app", lambda: installed)

    assert _prompt_and_install_tauri_app() == installed


def test_prompt_and_install_skips_non_interactive(monkeypatch) -> None:
    monkeypatch.delenv("AGEOS_INSTALL_APP", raising=False)
    monkeypatch.setattr("ageos.app.desktop._can_prompt", lambda: False)
...
def test_install_tauri_app_builds_and_installs_binary(monkeypatch, tmp_path) -> None:
    ...
    built = tmp_path / ".cache" / "ageos" / "app-target" / "release" / "ageos-control-center"
    ...
    monkeypatch.setattr("ageos.app.desktop._app_manifest_path", lambda: manifest)
    monkeypatch.setattr("ageos.app.desktop._run_app_deps_installer", lambda: None)
    monkeypatch.setattr("ageos.app.desktop.shutil.which", lambda name: "/usr/bin/cargo" if name == "cargo" else None)
    monkeypatch.setattr("ageos.app.desktop.Path.home", lambda: tmp_path)
    monkeypatch.setattr("ageos.app.desktop.subprocess.run", fake_run)

    installed = _install_tauri_app()

    assert installed == tmp_path / ".local" / "bin" / "ageos-control-center"
...
def test_run_app_deps_installer_honors_skip(monkeypatch) -> None:
    monkeypatch.setenv("AGEOS_SKIP_APP_DEPS", "1")
    run = Mock()
    monkeypatch.setattr("ageos.app.desktop.subprocess.run", run)
...
def test_launch_tauri_app_sets_url_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ageos.app.desktop._tauri_command", lambda: ["/bin/ageos-control-center"])
    monkeypatch.setattr("ageos.app.desktop.subprocess.run", fake_run)

    __import__("ageos.app.desktop", fromlist=["_launch_tauri_app"])._launch_tauri_app("http://127.0.0.1:9999/")

    assert captured["command"] == ["/bin/ageos-control-center", "http://127.0.0.1:9999/"]
    assert captured["env"]["AGEOS_APP_URL"] == "http://127.0.0.1:9999/"
