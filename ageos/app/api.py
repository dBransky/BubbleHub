...
from ageos.app.agents import delete_agent, stop_agent
...
def create_control_server(
    ...
    return AgeosControlServer((config.host, config.port), _handler_for(config), config, client_factory or SchedulerClient.local)


class AgeosControlServer(ThreadingHTTPServer):
    daemon_threads = True
...
def _handler_for(config: ControlApiConfig) -> type[BaseHTTPRequestHandler]:
    class AgeosControlHandler(BaseHTTPRequestHandler):
        server_version = "AgeOSControl/0.1"
        ...
        def _send_icon(self, name: str) -> None:
            allowed = {"ageos-icon.png", "nvidia.svg", "qwen.svg", "mistral.svg"}
...
def _control_server(server: object) -> AgeosControlServer:
    if not isinstance(server, AgeosControlServer):
...
def _static_asset(name: str) -> Path:
    candidates = []
    configured = os.environ.get("AGEOS_APP_STATIC_DIR")
    if configured:
    ...
    candidates.extend(
        [
            root / "app" / "static",
            Path(sys.prefix) / "share" / "ageos" / "app" / "static",
            Path("/usr/share/ageos/app/static"),
        ]
    )
    for directory in candidates:
        ...
        if path.is_file():
            return path
    raise FileNotFoundError(f"AgeOS app static asset not found: {name}")


def _icon_asset(name: str) -> Path:
    candidates = []
    configured = os.environ.get("AGEOS_APP_ICON_DIR")
    if configured:
        candidates.append(Path(configured).expanduser())
    root = Path(__file__).resolve().parents[2]
    candidates.extend(
        [
            ...
            Path(sys.prefix) / "share" / "ageos" / "app" / "icons",
            Path(sys.prefix) / "share" / "ageos" / "app" / "icons" / "models",
            Path("/usr/share/ageos/app/icons"),
            Path("/usr/share/ageos/app/icons/models"),
        ]
    )
    for directory in candidates:
        path = directory / name
        if path.is_file():
            return path
    raise FileNotFoundError(f"AgeOS app icon not found: {name}")
