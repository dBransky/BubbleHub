...
from bubblehub.engine.downloader import HfDownloader
...
def detect_hardware() -> object:
    from bubblehub.native import detect_hardware as native_detect_hardware
...
def _sandbox_inference_base_url() -> str:
    host = os.environ.get("BUBBLEHUB_SANDBOX_INFERENCE_HOST")
    port = os.environ.get("BUBBLEHUB_SANDBOX_INFERENCE_PORT")
    if not host or not port:
        raise RuntimeError("BUBBLEHUB_SANDBOX_INFERENCE_HOST and BUBBLEHUB_SANDBOX_INFERENCE_PORT must be set inside the sandbox")
...
def _parse_port(value: str) -> int:
    try:
        ...
        raise RuntimeError("BUBBLEHUB_SANDBOX_INFERENCE_PORT must be an integer") from None
    if port <= 0 or port > 65535:
        raise RuntimeError("BUBBLEHUB_SANDBOX_INFERENCE_PORT must be between 1 and 65535")
    return port
...
def default_max_output_tokens() -> int:
    ...
    try:
        ...
        raise RuntimeError("BUBBLEHUB_MAX_OUTPUT_TOKENS must be an integer") from None
    if parsed <= 0:
        raise RuntimeError("BUBBLEHUB_MAX_OUTPUT_TOKENS must be greater than zero")
    return parsed
