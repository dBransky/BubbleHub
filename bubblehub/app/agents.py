...
def _state_root() -> Path:
    if os.environ.get("BUBBLEHUB_STATE_DIR"):
        return Path(os.environ["BUBBLEHUB_STATE_DIR"]).expanduser()
    if os.environ.get("XDG_STATE_HOME"):
        return Path(os.environ["XDG_STATE_HOME"]).expanduser() / "bubblehub"
    if os.environ.get("HOME"):
        return Path(os.environ["HOME"]).expanduser() / ".local" / "state" / "bubblehub"
...
def _remove_persistent_agent(root: Path, agent_id: str) -> bool:
    agent_dir = root / ".bubblehub" / "agents" / agent_id
    ...
    if resolved_agent.parent != agents_dir:
        raise ValueError("persistent sandbox path escaped .bubblehub/agents")
...
def _stopped_agent_record(agent_id: str, metadata: dict[str, object]) -> dict[str, object]:
    ...
    enriched["has_persistent_sandbox"] = bool(root_dir and _is_persistent_agent_dir(Path(root_dir) / ".bubblehub" / "agents" / agent_id))
    return enriched
