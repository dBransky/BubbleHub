...
static int expect_request(const char *raw, const char *method, const char *url) {
    ...
    int rc = bubblehub_http_proxy_parse_request(raw, strlen(raw), &parsed);
    TEST_CHECK(rc == 0);
...
static int test_tcp_port_is_valid(void) {
    TEST_CHECK(!bubblehub_tcp_port_is_valid(0U));
    TEST_CHECK(bubblehub_tcp_port_is_valid(1U));
    TEST_CHECK(bubblehub_tcp_port_is_valid(BUBBLEHUB_HTTP_PROXY_DEFAULT_PORT));
    TEST_CHECK(bubblehub_tcp_port_is_valid(BUBBLEHUB_TCP_PORT_MAX));
    TEST_CHECK(!bubblehub_tcp_port_is_valid(BUBBLEHUB_TCP_PORT_MAX + 1U));
    return 0;
}
...
static int test_parse_request_variants(void) {
    ...
    bubblehub_http_proxy_request parsed;
    TEST_CHECK(bubblehub_http_proxy_parse_request("not-http\r\n", strlen("not-http\r\n"), &parsed) != 0);
    return rc;
}
...
static int expect_denied_response(int client_fd) {
    ...
    TEST_CHECK(strstr(buffer, "BubbleHub proxy denied the request") != NULL);
    return 0;
}
...
static int test_handle_client_denies_get(void) {
    ...
    int rc = bubblehub_http_proxy_handle_client(fds[1]);
    close(fds[1]);
    TEST_CHECK(rc == 0);
...
static int test_handle_client_uses_trusted_agent_id_not_env(void) {
    char *state_dir = test_mkdtemp_copy("bubblehub-http-proxy-policy-test");
    TEST_CHECK(state_dir != NULL);
    setenv("BUBBLEHUB_STATE_DIR", state_dir, 1);

    bubblehub_access_request always;
    ...
    TEST_CHECK(bubblehub_access_apply_policy("agt-victim-manifest", &always, "always") == 0);
    setenv("BUBBLEHUB_AGENT_ID", "agt-victim-manifest", 1);
    ...
    int rc = bubblehub_http_proxy_handle_client_for_agent(fds[1], "agt-real-manifest");
    close(fds[1]);
    TEST_CHECK(rc == 0);
    rc = expect_denied_response(fds[0]);
    close(fds[0]);
    unsetenv("BUBBLEHUB_AGENT_ID");
...
static int test_broker_prompts_when_pending_write_would_fail(void) {
    char *state_dir = test_mkdtemp_copy("bubblehub-http-proxy-broker-test");
    TEST_CHECK(state_dir != NULL);
    setenv("BUBBLEHUB_STATE_DIR", state_dir, 1);

    char manifest_path[1024];
    TEST_CHECK(bubblehub_access_manifest_path("agt-brokerwrite", manifest_path, sizeof(manifest_path)) == 0);
    ...
    if (pid == 0) {
        close(client_fds[0]);
        close(broker_fds[0]);
        int child_rc = bubblehub_http_proxy_handle_client_for_agent_broker(client_fds[1], "agt-brokerwrite", broker_fds[1]);
        close(client_fds[1]);
        close(broker_fds[1]);
...
static int test_serve_rejects_invalid_listen_port(void) {
    TEST_CHECK_EQ(bubblehub_http_proxy_serve(0U, -1), -EINVAL);
    TEST_CHECK_EQ(bubblehub_http_proxy_serve(65536U, -1), -EINVAL);
    TEST_CHECK_EQ(bubblehub_http_proxy_serve(70000U, -1), -EINVAL);
    return 0;
}
#endif
