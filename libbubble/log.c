...
static bubblehub_log_level_t g_log_level = BUBBLEHUB_LOG_LEVEL_ERROR;
static FILE *g_log_sink = NULL;
static int g_log_sink_owned = 0;
static int g_log_initialized = 0;

static bubblehub_log_level_t parse_level(const char *value) {
    if (value == NULL || value[0] == '\0') {
        return BUBBLEHUB_LOG_LEVEL_ERROR;
    }
    if (strcasecmp(value, "debug") == 0) {
        return BUBBLEHUB_LOG_LEVEL_DEBUG;
    }
    if (strcasecmp(value, "info") == 0) {
        return BUBBLEHUB_LOG_LEVEL_INFO;
    }
    return BUBBLEHUB_LOG_LEVEL_ERROR;
}
...
static const char *level_name(int level) {
    switch (level) {
        case BUBBLEHUB_LOG_LEVEL_INFO:
            return "INFO";
        case BUBBLEHUB_LOG_LEVEL_DEBUG:
            return "DEBUG";
        default:
            return "ERROR";
    }
...
static int sandbox_allows_log_file(const char *path) {
    ...
    if (getenv("BUBBLEHUB_SANDBOX") == NULL || strcmp(getenv("BUBBLEHUB_SANDBOX"), "1") != 0) {
        return 1;
    }
    if (path == NULL || path[0] == '\0') {
        return 0;
...
static int should_emit(int level) {
    ...
    if (level == BUBBLEHUB_LOG_LEVEL_ERROR) {
        return 1;
    }
...
void bubblehub_log_init(void) {
    g_log_level = parse_level(getenv("BUBBLEHUB_LOG_LEVEL"));
    g_log_initialized = 1;
}
