...
int main(int argc, char **argv) {
    ...
    for (; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--memory") == 0 && i + 1 < argc) {
            ...
            bubblehub_log_set_level(argv[++i]);
        } else {
            BUBBLEHUB_LOG_ERROR("unknown bubblehub-sandbox option", "%s", argv[i]);
            return 2;
        }
    }
    if (i >= argc) {
        BUBBLEHUB_LOG_ERROR("missing sandbox command", "");
        BUBBLEHUB_LOG_INFO(
            "bubblehub-sandbox usage",
    ...
    cfg.binary = argv[i];
    cfg.argv = &argv[i];
    return bubblehub_sandbox_run(&cfg);
}
