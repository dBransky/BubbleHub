...
static int add_env_or_home_cache_rule(int ruleset_fd) {
    const char *cache = getenv("BUBBLEHUB_CACHE");
    if (cache != NULL && cache[0] != '\0') {
        return add_path_rule(ruleset_fd, cache, read_rights());
    }
    return add_home_child_rule(ruleset_fd, ".cache/bubblehub", read_rights());
}

static int add_scheduler_state_rule(int ruleset_fd, int abi) {
    const char *state_path = getenv("BUBBLEHUB_SCHEDULER_STATE");
...
static int add_models_config_rule(int ruleset_fd) {
    const char *config_path = getenv("BUBBLEHUB_MODELS_CONFIG");
...
static int add_rootfs_rule(int ruleset_fd) {
    const char *rootfs_path = getenv("BUBBLEHUB_ROOTFS_DIR");
...
int bubblehub_landlock_apply_filesystem(const char *writable_dir, int allow_dns) {
    ...
    rc = add_home_child_rule(ruleset_fd, ".config/bubblehub", read_rights());
    if (rc != 0) {
        close(ruleset_fd);
        return rc;
    }
