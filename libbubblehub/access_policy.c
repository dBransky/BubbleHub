...
#define BUBBLEHUB_ACCESS_SCHEMA_VERSION 1U
#define BUBBLEHUB_ACCESS_MAX_POLICIES 128U
#define BUBBLEHUB_ACCESS_MAX_PENDING 128U
#define BUBBLEHUB_ACCESS_JSON_CAPACITY 65536U

typedef struct {
    bubblehub_access_request request;
    char policy[BUBBLEHUB_ACCESS_POLICY_SIZE];
} bubblehub_access_policy_entry;

typedef struct {
    char id[BUBBLEHUB_ACCESS_ID_SIZE];
    bubblehub_access_request request;
    double created_at;
} bubblehub_access_pending_entry;

typedef struct {
    char agent_id[BUBBLEHUB_ACCESS_AGENT_ID_SIZE];
    bubblehub_access_policy_entry policies[BUBBLEHUB_ACCESS_MAX_POLICIES];
    size_t policy_count;
    bubblehub_access_pending_entry pending[BUBBLEHUB_ACCESS_MAX_PENDING];
    size_t pending_count;
} bubblehub_access_manifest;
...
static void normalize_request(bubblehub_access_request *dst, const bubblehub_access_request *src) {
...
static int is_valid_agent_id(const char *agent_id) {
    ...
    if (len <= 4 || len >= BUBBLEHUB_ACCESS_AGENT_ID_SIZE) {
        return 0;
    }
...
static int access_owner_identity(uid_t *uid, gid_t *gid) {
    ...
    if (parse_owner_id(getenv("BUBBLEHUB_HOST_UID"), &env_uid) && parse_owner_id(getenv("BUBBLEHUB_HOST_GID"), &env_gid) && env_uid != 0) {
        *uid = (uid_t)env_uid;
        *gid = (gid_t)env_gid;
        return 1;
    }
...
static int state_root(char *buffer, size_t buffer_size) {
    const char *explicit_root = getenv("BUBBLEHUB_STATE_DIR");
    ...
    if (xdg_state != NULL && xdg_state[0] != '\0') {
        int written = snprintf(buffer, buffer_size, "%s/bubblehub", xdg_state);
        if (written < 0 || (size_t)written >= buffer_size) {
            return -ENAMETOOLONG;
        }
        int rc = mkdir_p(buffer);
    ...
    if (home != NULL && home[0] != '\0') {
        int written = snprintf(buffer, buffer_size, "%s/.local/state/bubblehub", home);
        if (written < 0 || (size_t)written >= buffer_size) {
            return -ENAMETOOLONG;
        }
        int rc = mkdir_p(buffer);
    ...
    int written = snprintf(buffer, buffer_size, "/tmp/bubblehub-%lu/state", (unsigned long)getuid());
    if (written < 0 || (size_t)written >= buffer_size) {
        return -ENAMETOOLONG;
    }
    int rc = mkdir_p(buffer);
...
static void manifest_init(bubblehub_access_manifest *manifest, const char *agent_id) {
...
static char *read_file(const char *path) {
    ...
    struct stat st;
    if (fstat(fd, &st) != 0 || st.st_size < 0 || st.st_size > (off_t)(BUBBLEHUB_ACCESS_JSON_CAPACITY * 4)) {
        close(fd);
        return NULL;
    }
...
static void parse_request_object(const char *object, bubblehub_access_request *request) {
    char value[BUBBLEHUB_ACCESS_PATH_SIZE];
    bubblehub_access_request raw;
...
static void parse_policies(const char *json, bubblehub_access_manifest *manifest) {
    const char *end = NULL;
    const char *cursor = find_array(json, "policies", &end);
    while (cursor != NULL && cursor < end && manifest->policy_count < BUBBLEHUB_ACCESS_MAX_POLICIES) {
        const char *object_end = NULL;
        const char *object = next_object(cursor, end, &object_end);
        if (object == NULL) {
            break;
        }
        bubblehub_access_policy_entry *entry = &manifest->policies[manifest->policy_count];
...
static void parse_pending(const char *json, bubblehub_access_manifest *manifest) {
    const char *end = NULL;
    const char *cursor = find_array(json, "pending", &end);
    while (cursor != NULL && cursor < end && manifest->pending_count < BUBBLEHUB_ACCESS_MAX_PENDING) {
        const char *object_end = NULL;
        const char *object = next_object(cursor, end, &object_end);
        if (object == NULL) {
            break;
        }
        bubblehub_access_pending_entry *entry = &manifest->pending[manifest->pending_count];
        parse_request_object(object, &entry->request);
...
static int load_manifest(const char *agent_id, bubblehub_access_manifest *manifest) {
    manifest_init(manifest, agent_id);
    char path[PATH_MAX];
    int rc = bubblehub_access_manifest_path(agent_id, path, sizeof(path));
    if (rc != 0) {
        return rc;
    }
    ...
    char parsed_agent_id[BUBBLEHUB_ACCESS_AGENT_ID_SIZE];
...
static void json_builder_init(bubblehub_json_builder *builder, char *data, size_t cap) {
    builder->data = data;
    builder->len = 0;
    builder->cap = cap;
    builder->failed = 0;
...
static void json_append_escaped(bubblehub_json_builder *builder, const char *value) {
    json_append(builder, "\"");
    if (value != NULL) {
...
static void json_append_request(bubblehub_json_builder *builder, const bubblehub_access_request *request) {
...
static char *manifest_to_json(const bubblehub_access_manifest *manifest) {
    char *json = malloc(BUBBLEHUB_ACCESS_JSON_CAPACITY);
    if (json == NULL) {
        errno = ENOMEM;
        return NULL;
    }
    bubblehub_json_builder builder;
    json_builder_init(&builder, json, BUBBLEHUB_ACCESS_JSON_CAPACITY);
    json_append(&builder, "{\n  \"version\": %u,\n  \"agent_id\": ", BUBBLEHUB_ACCESS_SCHEMA_VERSION);
    json_append_escaped(&builder, manifest->agent_id);
    json_append(&builder, ",\n  \"policies\": [");
    for (size_t i = 0; i < manifest->policy_count; i++) {
        const bubblehub_access_policy_entry *entry = &manifest->policies[i];
    ...
    for (size_t i = 0; i < manifest->pending_count; i++) {
        const bubblehub_access_pending_entry *entry = &manifest->pending[i];
        json_append(&builder, "%s\n    {\"id\":", i == 0 ? "" : ",");
...
static int write_manifest(const bubblehub_access_manifest *manifest) {
    char path[PATH_MAX];
    int rc = bubblehub_access_manifest_path(manifest->agent_id, path, sizeof(path));
    if (rc != 0) {
        return rc;
    }
...
static unsigned long request_hash(const bubblehub_access_request *request) {
...
static int request_equals(const bubblehub_access_request *left, const bubblehub_access_request *right) {
...
static int request_matches_rule(const bubblehub_access_request *rule, const bubblehub_access_request *request) {
...
static bubblehub_access_policy_entry *find_policy(bubblehub_access_manifest *manifest, const bubblehub_access_request *request) {
    bubblehub_access_policy_entry *best = NULL;
    int best_score = -1;
    for (size_t i = 0; i < manifest->policy_count; i++) {
        bubblehub_access_policy_entry *entry = &manifest->policies[i];
...
static int remove_pending(bubblehub_access_manifest *manifest, const bubblehub_access_request *request) {
    size_t out = 0;
    int removed = 0;
    for (size_t i = 0; i < manifest->pending_count; i++) {
...
static int add_pending(bubblehub_access_manifest *manifest, const bubblehub_access_request *request) {
    for (size_t i = 0; i < manifest->pending_count; i++) {
        if (request_equals(&manifest->pending[i].request, request)) {
            return 0;
        }
    }
    if (manifest->pending_count >= BUBBLEHUB_ACCESS_MAX_PENDING) {
        return -ENOSPC;
    }
    bubblehub_access_pending_entry *entry = &manifest->pending[manifest->pending_count++];
...
int bubblehub_access_apply_policy(const char *agent_id, const bubblehub_access_request *request, const char *policy) {
    if (!is_valid_agent_id(agent_id) || request == NULL || policy == NULL) {
        return -EINVAL;
    }
    bubblehub_access_request normalized;
    normalize_request(&normalized, request);
    char normalized_policy[BUBBLEHUB_ACCESS_POLICY_SIZE];
    ...
    bubblehub_access_policy_entry *entry = NULL;
    for (size_t i = 0; i < manifest.policy_count; i++) {
    ...
    if (entry == NULL) {
        if (manifest.policy_count >= BUBBLEHUB_ACCESS_MAX_POLICIES) {
            return -ENOSPC;
        }
...
int bubblehub_access_evaluate(
    ...
    *decision = BUBBLEHUB_ACCESS_DECISION_DENY;
    ...
    bubblehub_access_policy_entry *entry = find_policy(&manifest, &normalized);
...
static void append_pending_entry_json(
    bubblehub_json_builder *builder,
    const char *agent_id,
    const bubblehub_access_pending_entry *entry,
    int *first) {
