...
static int split_host_port(
    ...
    *port = default_port;
    if (port_start != NULL && port_start < value + value_len) {
        ...
        if (end == port_buf || *end != '\0' || !ageos_tcp_port_is_valid((uint32_t)parsed)) {
            return -EINVAL;
        }
...
static int parse_absolute_url(ageos_http_proxy_request *parsed, const char *target, size_t target_len) {
...
static int connect_to_upstream(const char *host, uint32_t port) {
    if (host == NULL || host[0] == '\0' || !ageos_tcp_port_is_valid(port)) {
        return -EINVAL;
    }
    char port_str[16];
...
static int write_denied_response(int client_fd) {
    static const char body[] =
        ...
        "Run `ageos dashboard` to review pending access requests.\n";
    char header[256];
...
static int policy_request_from_http(const ageos_http_proxy_request *parsed, ageos_access_request *request) {
...
int ageos_http_proxy_handle_client_for_agent_broker(int client_fd, const char *agent_id, int access_broker_fd) {
    ...
    if (access_broker_fd >= 0 && ageos_access_needs_prompt(agent_id, &request, &needs_prompt) == 0 && needs_prompt) {
        int broker_rc = request_broker_decision(access_broker_fd, agent_id, &request, &decision);
        if (broker_rc != 0) {
            AGEOS_LOG_INFO("http proxy access broker failed", "method=%s url=%s err=%s", parsed.method, parsed.url, strerror(-broker_rc));
        }
    } else {
        int policy_rc = ageos_access_evaluate(agent_id, &request, 0, &decision);
        if (policy_rc != 0) {
            AGEOS_LOG_INFO("http proxy access policy failed", "method=%s url=%s err=%s", parsed.method, parsed.url, strerror(-policy_rc));
            decision = AGEOS_ACCESS_DECISION_DENY;
        }
    }
    if (decision != AGEOS_ACCESS_DECISION_APPROVE) {
        AGEOS_LOG_INFO("http proxy denied", "method=%s url=%s", parsed.method, parsed.url);
        return write_denied_response(client_fd);
    }
    ...
    if (upstream_fd < 0) {
        AGEOS_LOG_INFO("http proxy upstream failed", "method=%s url=%s err=%s", parsed.method, parsed.url, strerror(-upstream_fd));
        return write_denied_response(client_fd);
    }
    AGEOS_LOG_INFO("http proxy approved", "method=%s url=%s", parsed.method, parsed.url);
...
int ageos_http_proxy_handle_client_for_agent(int client_fd, const char *agent_id) {
    return ageos_http_proxy_handle_client_for_agent_broker(client_fd, agent_id, -1);
}

int ageos_http_proxy_handle_client(int client_fd) {
    return ageos_http_proxy_handle_client_for_agent(client_fd, NULL);
}
