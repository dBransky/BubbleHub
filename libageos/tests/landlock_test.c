...
#ifndef __linux__
    extern int ageos_landlock_apply_filesystem(const char *writable_dir, int allow_dns);
    (void)ageos_landlock_apply_filesystem;
#endif
    return 0;
}
...
static int test_rejects_protected_writable_dir(void) {
    TEST_CHECK(ageos_landlock_apply_filesystem("/usr", 0) == -EPERM);
    TEST_CHECK(ageos_landlock_apply_filesystem("/etc", 0) == -EPERM);
    return 0;
}

static int test_applies_to_writable_temp_dir_in_child(void) {
    char *dir = test_mkdtemp_copy("ageos-landlock-test");
    TEST_CHECK(dir != NULL);

    pid_t pid = fork();
    TEST_CHECK(pid >= 0);
    if (pid == 0) {
        int rc = ageos_landlock_apply_filesystem(dir, 1);
        _exit(rc == 0 ? 0 : 1);
    }
