...
static int test_log_levels_and_file_output(void) {
    char *dir = test_mkdtemp_copy("ageos-log-test");
    TEST_CHECK(dir != NULL);
    ...
    ageos_log_write(AGEOS_LOG_LEVEL_INFO, __FILE__, __LINE__, "unit test message", "key=%s", "value");
    ...
    ageos_log_set_level("error");
    ageos_log_write(AGEOS_LOG_LEVEL_INFO, __FILE__, __LINE__, "filtered info", NULL);
    TEST_CHECK(read_file_contents(log_path, contents, sizeof(contents)) == 0);
    TEST_CHECK(strstr(contents, "filtered info") == NULL);

    ageos_log_write(AGEOS_LOG_LEVEL_ERROR, __FILE__, __LINE__, "visible error", NULL);
    TEST_CHECK(read_file_contents(log_path, contents, sizeof(contents)) == 0);
...
static int test_log_init_from_env(void) {
    setenv("AGEOS_LOG_LEVEL", "debug", 1);
    ageos_log_init();
    ageos_log_set_file(NULL);
    ageos_log_write(AGEOS_LOG_LEVEL_DEBUG, __FILE__, __LINE__, "debug after init", NULL);
    unsetenv("AGEOS_LOG_LEVEL");
    return 0;
}

static int test_sandbox_log_file_restriction(void) {
    char *dir = test_mkdtemp_copy("ageos-log-sandbox-test");
    TEST_CHECK(dir != NULL);
    ...
    setenv("AGEOS_WORKSPACE", dir, 1);
    ageos_log_set_file(allowed_path);
    ageos_log_write(AGEOS_LOG_LEVEL_ERROR, __FILE__, __LINE__, "sandbox allowed log", NULL);

    char contents[256];
    TEST_CHECK(read_file_contents(allowed_path, contents, sizeof(contents)) == 0);
    TEST_CHECK(strstr(contents, "sandbox allowed log") != NULL);

    ageos_log_set_file("/etc/passwd");
    ageos_log_write(AGEOS_LOG_LEVEL_ERROR, __FILE__, __LINE__, "blocked path", NULL);
