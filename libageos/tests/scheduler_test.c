...
static int setup_scheduler_state(void) {
    char *dir = test_mkdtemp_copy("ageos-scheduler-test");
    TEST_CHECK(dir != NULL);
    snprintf(g_scheduler_state_path, sizeof(g_scheduler_state_path), "%s/scheduler.state", dir);
    setenv("AGEOS_SCHEDULER_STATE", g_scheduler_state_path, 1);
    return 0;
}

static int test_configure_limits(void) {
    TEST_CHECK(ageos_scheduler_configure_limits(16.0, 8.0) == 0);
    return 0;
}

static int test_register_and_snapshot(void) {
    TEST_CHECK(ageos_scheduler_register_agent("agt-sched-test", 4242, "/usr/bin/test-agent", 0, "general") == 0);
...
static int test_deregister_agent(void) {
    TEST_CHECK(ageos_scheduler_deregister_agent("agt-sched-test") == 0);
...
static int test_model_lifecycle(void) {
    TEST_CHECK(
        ageos_scheduler_mark_model_loaded("test-model", "general", "llama", 4.0, 2.0, 9001, 8080) == 0);
    ...
    TEST_CHECK(ageos_scheduler_mark_model_unloaded("test-model") == 0);
    TEST_CHECK(ageos_scheduler_evict_model("test-model") == 0);

    snapshot = ageos_scheduler_snapshot_json();
    TEST_CHECK(snapshot != NULL);
...
static int test_queue_item(void) {
    TEST_CHECK(
        ageos_scheduler_add_queue_item("job-1", "model_load", "general", "queued-model", 5, "waiting for RAM") == 0);
...
static int test_admit_model_job(void) {
    ...
    char reason[256];
    TEST_CHECK(
        ageos_scheduler_admit_model_job("general", "new-model", 0, 1.0, 0.0, &allowed, state, sizeof(state), reason, sizeof(reason)) ==
        0);
...
static int test_admit_existing_model(void) {
    TEST_CHECK(
        ageos_scheduler_mark_model_loaded("existing-model", "general", "llama", 1.0, 0.0, 9002, 8081) == 0);
    ...
    TEST_CHECK(allowed == 1);
    TEST_CHECK(ageos_scheduler_evict_model("existing-model") == 0);
    return 0;
}

static int test_inference_chat_invalid_request(void) {
    char *response = ageos_inference_chat_json("not-json");
    TEST_CHECK(response != NULL);
    TEST_CHECK_CONTAINS(response, "\"error\"");
    ageos_scheduler_free_string(response);
    return 0;
}

static int test_invalid_agent_registration(void) {
    TEST_CHECK(ageos_scheduler_register_agent("", 1, "/bin/true", 0, "general") == -1);
    TEST_CHECK(ageos_scheduler_register_agent(NULL, 1, "/bin/true", 0, "general") == -1);
    return 0;
}
...
int main(void) {
    ...
    unsetenv("AGEOS_SCHEDULER_STATE");
    return rc;
}
